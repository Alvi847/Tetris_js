...
class RGBColor {
    ...
    static RGBColortoHex(rgb) {
        let hex = "#";

        if (Number.isInteger(rgb.r) && Number.isInteger(rgb.g) && Number.isInteger(rgb.b)) {
            const correct_rgb = RGBColor.correctValues(rgb);
            hex += correct_rgb.r.toString(16).padStart(2, '0');
            hex += correct_rgb.g.toString(16).padStart(2, '0');
            hex += correct_rgb.b.toString(16).padStart(2, '0');
            return hex;
        }
        else
            throw Error(`Invalid color format, RGB values must be integer numbers (${rgb.r}, ${rgb.g}, ${rgb.b}`);
    }
